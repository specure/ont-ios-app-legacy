...
extension CLLocation: RMBTFormat {
    ...
    func rmbtFormattedString() -> String {
        ...
        let timestamp = _cllocation_timestampFormatter.string(from: self.timestamp)
        return String(format: "%@ %ld° %.3f' %@ %ld° %.3f' (+/- %.0fm)\n@%@",
            latDirection, latDegrees, latMinutes, longDirection, longDegrees, longMinutes,
            self.horizontalAccuracy, timestamp)
    }
