...
///
let NS_FONT_ATTRIBUTE = NSAttributedString.Key.font// "NSFontAttributeName"

///
let _uitableviewcell_font = UIFont.boldSystemFont(ofSize: 17)

///
let cellDetailTextFontAtrribute = [NS_FONT_ATTRIBUTE: _uitableviewcell_font]

///
let cellTextFontAttribute = [NS_FONT_ATTRIBUTE: _uitableviewcell_font]
...
extension UITableViewCell: RMBTHeight {
    ...
    class func rmbtApproximateOptimalHeightForText(_ text: String?, detailText: String?) -> CGFloat {
        ...
        if let t = text {
            textSize = t.size(withAttributes: cellTextFontAttribute)
        }

        var detailTextSize = CGSize(width: 0, height: 0) // default
        if let t = detailText {
            detailTextSize = t.size(withAttributes: cellDetailTextFontAtrribute)
        }
